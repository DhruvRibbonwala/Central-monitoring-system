...
pub = rospy.Publisher('r4/vel_controller/cmd_vel',Twist,queue_size=1)
...
def get_current_positions(posedata):
    global cp
    cp = [posedata.pose4.x,posedata.pose4.y,posedata.pose4.theta]
    

def get_viapoints(wpoints):
    global wp,change
    last = wp
    wp = [wpoints.pose4.x,wpoints.pose4.y,wpoints.pose4.theta]
    if last != wp:
        change = True
