...
pub = rospy.Publisher('r2/vel_controller/cmd_vel',Twist,queue_size=1)
...
def get_current_positions(posedata):
    global cp
    cp = [posedata.pose2.x,posedata.pose2.y,posedata.pose2.theta]
    

def get_viapoints(wpoints):
    global wp,change
    last = wp
    wp = [wpoints.pose2.x,wpoints.pose2.y,wpoints.pose2.theta]
    if last != wp:
        change = True
